...
import { erc20Abi } from '../polymarket/erc20Abi';
...
export const checkUSDCAllowance = async (
    ...
    const provider = new ethers.providers.JsonRpcProvider(ENV.RPC_URL, POLYGON_NETWORK);
    const usdcContract = new ethers.Contract(ENV.USDC_CONTRACT_ADDRESS, erc20Abi, provider);
...
export const approveUSDC = async (
    ...
    const usdcContract = new ethers.Contract(ENV.USDC_CONTRACT_ADDRESS, erc20Abi, wallet);
...
export const checkWalletStatus = async () => {
    ...
    try {
        ...
        const walletFromKey = new ethers.Wallet(ENV.PRIVATE_KEY, provider);
        const addressFromKey = walletFromKey.address;
        
        console.log('\n📋 Configuration:');
        console.log(`   PRIVATE_WALLET (from .env):  ${ENV.PRIVATE_WALLET}`);
        console.log(`   Address from PRIVATE_KEY:  ${addressFromKey}`);
        
        if (addressFromKey.toLowerCase() !== ENV.PRIVATE_WALLET.toLowerCase()) {
        ...
        console.log(`   Checking configured USDC contract: ${ENV.USDC_CONTRACT_ADDRESS}`);
